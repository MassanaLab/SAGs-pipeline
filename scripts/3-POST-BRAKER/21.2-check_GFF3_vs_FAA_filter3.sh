...
FAA_DIR="lustre/aleix_gff_process_big2_${W}_filter3/final_faa"
PROC_DIR="lustre/aleix_gff_${W}_process_out_filter3"
...
# find every *_filter3_genes.faa and derive the sample name
while IFS= read -r -d '' faa; do
  (( total++ ))
  base="$(basename -- "$faa")"
  sample="${base%_filter3_genes.faa}"
...
done < <(find "$FAA_DIR" -maxdepth 1 -type f -name '*_filter3_genes.faa' -print0 | sort -z)
